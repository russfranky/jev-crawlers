// lib/jev.mjs — typed Jev judgments for crawl-judge.
//
...
export function packState(node, opts = {}) {
  ...
  // Evidence: keep the first 8, note the rest.
  ...
  if (keptEvidence.length) {
    lines.push('EVIDENCE:');
    for (const e of keptEvidence) lines.push(`- ${String(e).slice(0, 500)}`);
  }
...
export async function judgeNode(node, set, opts = {}) {
  ...
  if (set.status === 'proposed') {
    process.stderr.write(`crawl-judge: WARNING set "${opts.setName || '?'}" is proposed/un-calibrated; result is advisory only.\n`);
  }
